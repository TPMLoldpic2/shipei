...
## v7：connect.txt 外部連結設定
現在「活動資訊」與「問卷回饋 / 問卷調查」不用再改 index.html。

直接修改網站根目錄的 `connect.txt`：

```txt
ITEM\tLINK
活動資訊\thttps://example.com/activity
問卷調查\thttps://example.com/survey
```
...
| NO | ITEM | LINK |
| -- | ---- | ---- |
| 1 | 活動資訊 | https://example.com/activity |
| 2 | 問卷調查 | https://example.com/survey |
```

注意：
- 網址建議使用 `https://` 開頭。
- LINK 留空時，點底部按鈕會提示「網址尚未設定」。
- `connect.txt` 採 network-first，每次重新開啟頁面會優先讀取伺服器最新版。
