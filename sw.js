const CACHE_VERSION = "webar-pwa-v7";
...
const PRECACHE = [
  ...
  "./icons/apple-touch-icon-180-v6.png"
];
...
self.addEventListener("fetch", event => {
  ...
  // connect.txt 是可直接手動修改的連結設定：永遠先抓最新版，離線才用快取。
  if (url.origin === self.location.origin && url.pathname.endsWith("/connect.txt")) {
    const canonical = new Request(new URL("./connect.txt", self.registration.scope).href);
    event.respondWith(networkFirst(request, canonical));
    return;
  }
